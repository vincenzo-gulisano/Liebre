 ...
 * {@link ExponentialBackoff#factory(int, int, int)} to create thread-local instances of the object
 * with the same configuration. </p> <br/>
 * <p>
 * Every time {@link ExponentialBackoff#backoff()} is called, the calling thread sleeps for a random
 * duration in the interval {@code [0, limit]} milliseconds. The range of the limit is defined in
 * the constructor (parameters {@code min, max} and it is exponentially increased for every {@code
 * retries} consecutive calls for the backoff function. A similar exponential reduction of the limit
 * happens when consecutive calls to the relax function happen.
 * </p>
 */
public class ExponentialBackoff implements Backoff {
  ...
  /**
   * Create a factory that generates {@link ExponentialBackoff} instances with the given
   * configuration.
   */
